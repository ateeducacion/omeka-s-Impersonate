...
# Generate the Impersonate-X.X.X.zip package
package:
	@if [ -z "$(VERSION)" ]; then \
	...
	@echo "Creating ZIP archive: Impersonate-$(VERSION).zip..."
	composer archive --format=zip --file="Impersonate-$(VERSION)-raw"
	@echo "Repacking into proper structure..."
	mkdir -p tmpzip/Impersonate && unzip -q Impersonate-$(VERSION)-raw.zip -d tmpzip/Impersonate && \
	cd tmpzip && zip -qr ../Impersonate-$(VERSION).zip Impersonate && cd .. && rm -rf tmpzip Impersonate-$(VERSION)-raw.zip
